...
| K8 | **HALF CLOSED (PR-26a)**: the headlight points FORWARD with a glass lens, bracket and guard — it was a `drum_fitting`, and `revolve::drum` revolves about Y, so the one part whose job is to point somewhere lay on its side shining at the sky. Still open: the cupola and three hatches are bare drums, and the repo still has ZERO hinges/handles/latches | `the_headlight_faces_forward_and_shows_glass`; `t54_details.rs` | PR-26a done / **PR-26b** |
