...
| **I2** | ~~**A refusal does not say what refused it.**~~ — **FIXED (wave 2).** While BLOCKED the range readout still prints the distance to the TARGET (327 M in the report) — a tank the gun cannot reach. It is the sight's one outright lie, and the amber X that does mark the real impact carries no number and no line back to the crosshair. | `hud/reticle_readouts.rs:123` |
| **I3** | ~~**The sight ray probes the world with a zero-radius body; the shell carries its calibre.**~~ — **FIXED (wave 3).** `aim_point_with_sweep` passes `projectile_radius_m: 0.0` while the trace passes `collision_radius_m()`. That difference alone is **35 of the 36** Bystra refusals where the target is not cut anywhere in the picture: the ray threads a barn roof by ~7 cm and the shell grazes it. | `aim.rs:122` |
| **I4** | ~~**Two greens, one place, unrelated meanings.**~~ — **FIXED (wave 4).** `RETICLE_LOADED` `[0.40, 0.90, 0.42]` (breech shut, 0.95 s) against `RETICLE_PEN` `[0.35, 0.85, 0.40]` (your shell goes through) — 5 % apart and indistinguishable. The report read the loaded ring as the penetration verdict, which is exactly what the colour says. The code comment already admits it: the distinct bytes exist "so the locks can tag it apart", i.e. for tests, not for eyes. | `hud/reticle_overlay.rs:45` |
...
| **J3** | ~~**The meadow is still BAKED on every crater.**~~ — **FIXED.** 133–249 ms of worker CPU in the late game, resident on one core, almost always producing a byte-identical mesh. J1 stopped the upload, not the bake. Bucketing turned out to be unnecessary: the meadow reads the ledger only through the cards a burst mows and the ground a bowl moves, so a coarse footprint of where the meadow STANDS answers "can this crater have changed anything?" in microseconds. Craters that entered or left the ledger are tested against it — removals count, the ledger recycles — and the bake does not start unless one of them reaches occupied ground. | `scene_build/src/grass_cards.rs` |
| **J4** | ~~**Running gear is rebuilt for all 14 tanks every frame into fresh unsized `Vec`s.**~~ — **FIXED, and the diagnosis corrected.** The allocation was the smaller half of it: measured with the two forms INTERLEAVED, reusing buffers is **−12 %** of the gear build, not the bulk of it — the rest is 2,856 matrix constructions. Measured sequentially the same pair disagreed by a factor of two in whichever order they ran, which is the trap the config rotation exists for and which this entry's original 0.29 ms had walked into. Both are done: the build writes into a buffer the caller keeps, and the fleet frame is sized up front. | `running_gear_place.rs` |
| **J5** | ~~**A wreck costs what a live tank costs.**~~ — **FIXED.** There is no cheaper path for a destroyed hull: it still gets full animated running gear and per-wheel terrain sampling every frame, for the rest of the match. Not an accumulation (14 tanks are always 14), but late in a battle most of that work animated vehicles that will never move again. The placements are hull-local, so they are now cached per tank against exactly what shapes them — track scroll, wheel travel, top-run sag, thrown tracks — and a hull that moved none of those reuses last frame's. The generalisation is worth more than the wreck case it came from: a tank holding a firing position stops rebuilding too. | `client/src/vehicle/gear_cache.rs` |
