...
| G5 | ~~The gun marker was slow and nagging~~ — **FIXED.** Its fade band was fixed clip distance: ~7–15 mrad in third person but 1–2 mrad under 6.9× sniper, so it stayed lit through the whole exponential tail of the turret's fine lay, most nagging where aiming is most deliberate. The band is now a fraction of the live dispersion ring. It was also a *circle* in a sight where every circle means this gun's dispersion; it is a diamond now. | `hud/reticle_marks.rs:22` |
...
| G7 | **A settled ring hides inside the crosshair.** The crosshair arms reach 0.020 clip; a T-54's settled third-person ring is ~0.018 (2.9 mrad through an 18° view). At full convergence — the moment the sight most wants to be read — ring and crosshair merge into one ⊕ blob (visible in the `CONVERGED` cell of the contact sheet). Either the arms shrink or they gain a centre gap. | `hud/reticle_overlay.rs:98` |
| G8 | **The ring's outline is one-sided.** The dark underlay twin is drawn only OUTSIDE the ring (`radius + 0.0018`), so on pale straw the inner edge has no contrast at all. The comment claimed a twin "on each side" and has been corrected to match the code; the code is what should change. | `hud/reticle_marks.rs:44` |
| G9 | **The denial pulse now shares a colour with the state it most often answers.** With the loading arc red, a refused click while reloading pulses red over red — the two read as one thick smear on the contact sheet. The pulse is unambiguous where it matters most (a refusal with a GREEN loaded ring: dead gun, empty rack), but the common case lost contrast. | `hud/reticle_marks.rs:85` |
| G10 | **The readouts have fixed offsets while the ring breathes.** Distance and the pen/armour millimetres sit at +0.18/−0.115 clip; a bloomed ring reaches 0.35 and swallows them. The same corner also collides with the incoming-hit arc, and an open-sky sight ray prints a fictitious "1200 M" (the ray's own cap, not a target). | `hud/reticle_readouts.rs:64,102` |
...
| G12 | **Unproven, but observed: a battle's first seconds may not be deterministic under load.** A test that ran `run_fixed_ticks(150)` from a fresh `ClientApp` and then asked whether the gun had settled passed 2/2 solo and failed 2/2 inside the full `-p client --lib` run — the same seed, the same ticks, no RNG in the path. The suspect is the speculative map bake (`map_prebake`, a background thread): under suite load the first ticks may run against a different battlefield state than they do solo, which would make the hull settle — and therefore the movement bloom — depend on wall-clock. I sidestepped it (the test now drives dispersion through an authoritative snapshot, which is what it meant to lock) rather than diagnosing it, so this is an observation, not a verdict. It is the third load-sensitive client test on this list after F8/F9; if the cause is real it is a *game* bug, not a test bug. | `app/mod.rs` `map_prebake` |
