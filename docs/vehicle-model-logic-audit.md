...
| 17 | ~~Running-gear discs INSIDE each other~~ **FIXED for the Centurion, gated fleet-wide**: its three Horstmann bogies merged 0.20 m — ⌀0.80 wheels on a 0.60 pitch — and the middle return roller sat 0.065 m inside both centre wheels. Root cause was not the stations but the WHEEL SIZE: `cy` (the axle) was averaged out of `top_y..bottom_y`, so a vehicle whose ROLLERS carry the top run had to grow its wheels until they bridged a belt they never touch. `TrackShape::axle_y` now states the axle outright; the Centurion runs its real ⌀0.61 wheels at 0.66 pitch (5 cm of daylight), and `fleet_running_gear.rs` checks every disc against every disc sharing its plane. Recorded debt: T-34-85 road wheel ↔ end wheel 0.09 m; IS-3 ground run drawn 0.08 m above its authored belt bottom (same averaged-axle root, its own shape call). Three German benchmarks stated this rule locally ("discs must not merge") — it was never run where nobody expected an overlap. | Centurion (fleet gate) | `running_gear.rs` axle derivation + blueprint | logic |
