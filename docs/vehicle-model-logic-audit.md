...
| 6 | Mantlet collar identical across guns (one bulge profile, scaled) — **muzzle half solved with #4**; the T-54's flat oval and the **KV-1's tall ZiS-5 mask** (#18) are authored, so the pattern and the helper both exist; per-vehicle mantlet MASSES for the German four (Walzenblende, Turmblende, Saukopf collar, G-blende) remain W1 dossier work | fleet | `build_gun` mantlet profile | clone |
...
| 18 | ~~KV-1 shipped on green gates with four close-up defects~~ **FIXED**: the first vehicle to go through the new gate as it was written — built, reviewed at garage distance, and the review's findings fixed rather than filed. (a) the mantlet was the fleet's generic bulge; the KV now has an authored cast MASK, taller than wide, the opposite proportion to the T-54's flat oval — **this closes the KV's half of #6**, the German four remain open; (b) unbroken plate everywhere (defect **D15** in the art-direction register); the tank now carries pistol ports and lifting lugs cast into the turret, spare track racked ON the bow plane, a tow cable and two stowage boxes per fender shelf, and three grab rails a side with a bracket at each end; (c) the casting read as a soft blob because the loaf's straight walls were blended into its caps — hard seams cost zero triangles and it now reads flat-walled; (d) a plain smooth idler beside six spoked road wheels, which was never a KV bug: `idler_unit_mesh` took no per-vehicle input at all, so the whole fleet shared one drum (#14 de-cloned the shoes and road wheels and stopped at the end wheels). `IdlerFace` follows the same serde-default mechanism, so the other eight are untouched. Also fixed: the torsion swing arms were hardcoded to the T-54's 810 mm wheel and now scale to their own — the KV's 600 mm and the IS-3's 550 mm gear were wearing a T-54's arm. | KV-1 | `recipes/kv1.rs`, `turret_fittings.rs`, `running_gear_{arms,end_wheels}.rs` | clone + logic |
