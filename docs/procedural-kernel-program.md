...
| Closed paths: covers, kit lines, hoses, rails, welded bead paths | `sweep` — in production for the T-54 gun cover and kit lines (`t54_gun_cover.rs`, `t54_kit.rs`) and the weld bead (`detail::weld`) | A cross-section follows a stable path frame. **Tracks do not route here**: a shoe is a box-prism link (`vehicle_geometry::running_gear_geom`) instanced along the belt path — K12 |
