...
| The AI battle | `new_ai_battle`: one human, 29 bots, 900 s; no socket (M2); the app's offline battle and the garage's BATTLE since M3 | the results screen's mode word, and the online switch beside it — M7b |
...
| ~~M3~~ | ~~**The AI battle on the button**~~ — **CLOSED (2026-09-06)**, the owner's call at the day's end: the app's offline battle and the garage's BATTLE deploy the AI battle (`new_ai_battle` — 15v15, the player and twenty-nine marked bots, 900 s, no socket); the button keeps its word and the frame its golden until M7b puts the online queue beside it (one re-bless, not two) and the results screen names the mode. Three budgets sized for fourteen went with it: the FX tracer allowance, the kill feed's memory (16 → 32) and the reliable lane's join seed (`MAX_PENDING_COMBAT_EVENTS` 1 024 → 2 048). The thirty-hull frame on the MX330 is still unmeasured (M5b) — the owner chose the mode before the number | `crates/apps/client/src/app/mod.rs`, `crates/apps/client/src/app/garage/actions.rs`, `crates/apps/client/src/fx/budget.rs`, `crates/apps/client/src/app/battle_intel.rs`, `crates/runtime/battle_host/src/remote_events.rs` | `the_ai_battle_is_fifteen_against_fifteen_and_needs_no_socket` (the app's session is local, `AiBattle`, thirty hulls, one human), the garage's fresh-deploy lock at thirty and 900 s, `the_join_seed_fits_the_reliable_lane` at the largest format |
