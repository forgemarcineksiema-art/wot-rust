...
## STATUS (2026-07-23) — read this first if you are picking the work up
...
**Program B (flora): FL-1..FL-5 COMPLETE** (#295-#298 plus the map-integration change): UV lane,
foliage atlas + alpha cutout honest in color/shadow/SSAO, alpha-coverage-preserving sRGB
mips, the CC0 importer (`tools import-flora`, license gate + loud decimation + multi-texture
compositing), runtime catalog + `FloraTree`/`FloraPine` scenery kinds, and deterministic
imported-tree scatters on all four maps.
Ostrogorsk's orchard, park, boulevard avenue and elevator pines use the imported assets;
the other maps retrofit their broadleaf and alpine-pine scatters. Look gate verdict (see
`flora_probe` example): tree ACCEPTED, pine ACCEPTED, bush REJECTED (bad source model —
do not author `FloraBush` on maps). The FL-5 runtime gate is now measured at 1080p on the
minimum-spec MX330: two consecutive full-scatter medians were 14.545 and 13.281 ms/frame
(118 imported instances), below the 16.667 ms 60 FPS budget.

**Remaining, in order:**
1. **Sourcing** — a real CC0 bush and a TEXTURED birch (first candidates refused by the
...
2. Doctrine follow-ups: map rotation, an Ostrogorsk bot battle test à la `orliny_battle.rs`.
