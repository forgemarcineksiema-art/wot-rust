...
| 15 | **Humans on one side only** (found 2026-09-06, `docs/game-modes.md` finding 1) | `random_7v7_setup_for_humans` (`battle_host/src/setup.rs`) reserves the first `humans` slots of TEAM ONE and fills team two with bots; the lobby's "full" is one team's seats (`LOBBY_FULL_PLAYERS = SEATS_PER_TEAM`, `remote.rs`) — the dedicated host hosts co-op against bots, never humans against humans | seats dealt across both teams by the queue's rules (game-modes R4), "full" = both teams' seats; the per-viewer filter needs nothing | M (game-modes M6, before any human-vs-human test) |
| 16 | **The largest format against the transport** (found 2026-09-06, `docs/game-modes.md` finding 3) | a saturated 14-tank snapshot is 5 245 B of 32 200 B (5/28 fragments, 102 KiB/s per client at 20 Hz, measured by `snapshot_budget.rs`); thirty tanks ≈ 11 240 B, 35 %, ten fragments — fits, breaks the quarter rule, and loses one snapshot in five at 2 % packet loss | row 9's delta snapshots / smaller payload BEFORE 15v15 goes online with humans (moved ahead of N5 for that format); the budget lock per format (7v7 a quarter, 15v15 half at ≤ 10 fragments) | M–L (game-modes M5) |
