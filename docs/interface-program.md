...
| P8 | **Escape offers no exit from a cold garage**; the pause menu has two entries — **half done (2026-09-06)**: the escape menu offers SETTINGS · KEY BINDINGS · HUD EDITOR · EXIT TO GARAGE · STAY, and the KEY BINDINGS page landed (`hud/shell.rs`, golden `shell_keybinds_conflict`: one context at a time, Enter listens for the next key, R resets, a shared key named on both rows, every change written to `keybinds.json`); the cold garage's Esc and QUIT are the rest | `crates/apps/client/src/app/garage/mod.rs:255-259`, `crates/apps/client/src/hud/pause_menu.rs:33-38` | P | `escape_always_offers_a_way_out` — from every screen QUIT or the battle in at most three presses (absorbs U5 with P1, P6 and P7) |
