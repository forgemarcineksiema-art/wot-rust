...
| H9 | Floating numbers carry no semantic colour and owe their restyle under S7 — and the owner's rule that they are quiet: „nie natrętnie kolorowo, przesadnie" | `crates/apps/client/src/hit_indicator.rs` | H | one muted tone per outcome family, no scale or bounce animation, no stacking; the S7 and A6 locks green after the restyle; a chroma ceiling on the number's tokens in the theme lock; `kill_feed_and_numbers` |
...
| H26 | The HUD's cost has a target and no number. **2026-09-06**: the synthetic worst case re-measured after F3's SDF glyphs — p50 0.289 ms, p95 0.318 ms cold on the MX330 (F1: 0.179) — the fragment shader fetches the field and its derivative for every fragment to keep derivatives in uniform control flow; neither the F2 nor the F3 gate ran the lock, so master carried it red from #725 to the re-measure. FLOOR 0.27 → 0.43 on purpose, with the number; the TARGET 0.5 holds. First shave when the real HUD's row needs it: the per-fragment field fetch (a uv-derivative anti-aliasing width from `textureDimensions`, the fetch inside the glyph branch) | `crates/render/renderer_wgpu/tests/hud_budget.rs`, `crates/render/renderer_wgpu/src/shaders/hud.wgsl` | H (last) | the F1 lock's FLOOR at or under the 0.5 ms TARGET, or the debt recorded per element with its measurement |
| H27 | **The reticle carries every good idea it ever had**, and nobody asked which of them the player needs in the second they look at it: the dispersion ring, the gun marker, the ready ring, the denied flash, the reload number, the pen and millimetre readouts, the distance, the block form, the arc glyphs, the impact X, the spot brackets, the scope surround — each was a good decision on its own (the sight audit of 2026-08-05 found three geometries answering one question) | `crates/apps/client/src/hud/reticle_overlay.rs`, `crates/apps/client/src/hud/reticle_marks.rs`, `crates/apps/client/src/hud/reticle_readouts.rs`, `crates/apps/client/src/hud/demo_strip.rs` | H (its own PR, with the A lane) | an audit of every mark against the four questions (where will I hit, will it penetrate, may I fire, how long until I may): a mark stays only with the question it answers written beside it in the code, and merges with another mark that answers the same one; the rest goes; closes with a `reticle_strip` frame the owner signs, the A-lane locks re-pinned deliberately in that PR with the before/after, never quietly |
