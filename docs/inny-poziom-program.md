...
| Q5 | **The physics arithmetic depends on which crates are in the build.** Found closing A8: `predict::parity_tests::ramming_a_live_hull_does_not_buzz_the_camera` passes under `cargo test --workspace --all-targets` (the gate) and fails under `cargo test -p client` (16 corrections against 12) — on clean master, 3 of 3. `cargo tree -e features -i glam` shows why: the client-only build resolves `glam` with `nostd-libm` (through `glamx` ← `parry3d` ← `physics`), the workspace build adds `glam/default` (`std`) from other members, and the two trig backends do not agree to the last bit. The same mechanism can put the dedicated `server` binary on a different backend from the `client` — the predictor's parity and the replay-exact locks assume one arithmetic | `Cargo.toml` `[workspace.dependencies] glam`, `crates/runtime/physics/Cargo.toml` (`parry3d`) | Q lane (first) | one backend pinned in the workspace for every member (the cross-platform replay locks want `libm` everywhere — pure Rust, identical on every platform — not `std`'s intrinsics); a quality gate asserting `cargo tree -p server` and `-p client` resolve `glam` to the same feature set; the ramming lock re-pinned on that backend |
