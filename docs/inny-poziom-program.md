...
| ID | Defect | Evidence | Wave | Closes when |
|---|---|---|---|---|
| A1 | **The reticle and the server resolve penetration by different code.** `penetration_from_outcome` drops `thickness_scale` (destructured away, `hud/reticle.rs:219`) and resolves a bare track/skirt band while the server sums the spaced stack plus the hull side (`resolve.rs:167-231`). A 20 mm track reads green, the server charges band + belt → "0" + TRK. No test locks reticle verdict == server verdict | `client/src/hud/reticle.rs:204-235` vs `sim/src/combat.rs:779-849` | W1 | one `resolve_impact_penetration` in `game_core` called by both; a property test over 10 000 traced impacts asserting `hint.penetrates == server.penetrated` |
...
| A3 | **BLOCKED is nearly mute in third person, and a pitch limit looks like a wall.** The pen hint, impact X and leader are sniper-gated (`reticle_overlay.rs:188,205`); in TPP a grey glyph is all. A target below the gun's depression clamps `in_arc=false` → the same Blocked form, the shell into the dirt — indistinguishable from cover | `app/aim.rs:191-195`, `hud/reticle.rs:175,183` | W1 | the block distance, impact X and leader drawn in both modes; a distinct pitch-limit glyph; locks per mode |
...
| A5 | **Two silent refusals.** A fire input while the session is not ready is dropped before any feedback runs (`loop_step.rs:53-56` vs `:72-74`); the 30-tick deploy shield has a latch that is never set | `app/loop_step.rs`, `app/input.rs:109`, `app/mod.rs:392` | W1 | every refusal reaches the UiReject knock; lock over the refusal enum |
| A6 | **A literal "0" is drawn for five outcomes** (ricochet, non-pen, near-pen, shatter, track stack) — `hit_indicator.rs:98-108` pushes `damage_hp` unconditionally | `client/src/hit_indicator.rs`, `hud/number.rs:23-26` | W1 | the outcome word replaces the number for zero-damage outcomes (RICOCHET / NO PEN / TRACKED / ABSORBED / SHATTER) in the outcome colour; lock: "0" never rendered |
...
| Q2 | **No hitch instrument on a live battle.** `perf_capture` has p50/p95/p99/max on a static, sim-free path; `battle_age_cost` sizes the ground row but not the statics row; nothing measures "the longest frame and what ran in it" under sim + render + audio + network together | `client/examples/probe/perf_capture.rs:613,672`, `battle_age_cost.rs:237-285` | Q lane (first) | `battle_age_cost` extended: the statics row, and a live `battle_host` session firing HE on a schedule with the wall-clock-minus-fence percentile method — p99/max on the frame that harvests a real crater or collapse |
