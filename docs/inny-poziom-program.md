...
| A10 | **The turret is steered by a proportional controller with a low gain.** `TURRET_TRACK_GAIN = 5` / `GUN_TRACK_GAIN = 6`: full rate only above 11.5° (turret) / 9.5° (gun) of error, then an exponential tail (τ 455–714 ms turret, 333 ms gun); tracking a moving sight leaves a PERMANENT lag of `11.5° × (sweep rate ÷ turret rate)` — 5.7° at half rate — which in metres is `crossing speed ÷ (rate × gain)`, range-independent: a T-54 tracking a 54 km/h crosser holds its gun 7.1 m behind the crosshair forever, more than the ballistic lead (5.1 m at 300 m). The sim is a clean rate limiter with no inertia and the player\x27s own gun carries zero network delay (locally predicted), so the whole felt lag is this controller. Owner 2026-09-02 (playtest): "ciężko nadgonić celownik, a żeby strzelić w ruchomy pojazd to już w ogóle" | `client/src/app/prediction.rs:9,193`, `app/reticle.rs:8,74` | W1 | time-optimal command (`aim::rate_command_to_land`): full rate until the remaining error fits in one tick, then exactly the remainder — lands without overshoot in `ceil(error / (rate·dt))` ticks and tracks any sweep under the turret rate with at most one tick of lag; locks on the pure function and on a sweeping sight |
| A11 | **Traverse rates are half the genre\x27s, and every hull out-turns its own turret.** Fleet 16–25 deg/s (T-54 24 vs ~48 WoT-typical; 180° in 7.5 s, Tiger II 11.3 s), spread only 1.57×; hulls 18–46 deg/s, so steering strips world bearing faster than the turret recovers it (T-54: −20.6 deg/s net) | `game_core/src/modules/catalog_*.rs` | W1 | genre-class per-vehicle rates (T-54 48, T-34-85 46, Centurion 36, Panther II 30, IS-3 28, Tiger I 26, Tiger II 23 deg/s); lock: no rotating turret under 22 deg/s, every turret ≥ 0.75 of its hull\x27s turn rate, fleet spread ≥ 2×; replay fixtures that traverse re-pinned deliberately |
...
| Q7 | **The battle frame has drifted far above Q3's measurement.** 2026-09-02, MX330, `perf_capture` at the shipped 1×, two interleaved rotations within an hour of a full `-Deep` gate (a warm box): "full scene" 21.9 / 23.4 ms p50, "full + 7v7" 29.8 / 28.1 ms p50, readback fence alone 6.4 ms — against Q3's 11.95 / 15.38 ms of 2026-08-09 on the same box. Not attributable yet: the thermal state, or the rows landed since (Drzewa 3.0, the tree lines of F2/F3, the light W1 campaign, Z1's theatre) | `perf_capture` logs of 2026-09-02 (run 1 and run 2, S1) | W1 | a COLD rotation on the same box before the next renderer row lands, then the per-pass table names the pass; the one-look floor is 60 FPS on the MX330 and the number that fits is not the budget — if the drift is real it goes before every remaining S/F/N/H row |
