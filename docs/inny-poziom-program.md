...
| A9 | **A spotted enemy is unmarked in the scope.** Spotting is cull-only (`app/aim.rs:335`): no outline, bracket or nameplate; at 8° a T-54 at 300 m is ~90 px wide against a field of the same value | `app/aim.rs:335`, `hud/health_bar.rs` (the floating HP bar already projects spotted hulls) | W1 | a faint corner bracket on the projected hitbox of every spotted enemy, sniper mode only, in the reticle's neutral tone; lock: the bracket draws for a spotted hull and never for an unspotted or friendly one; measured for legibility on `prokhorovka_sniper_contact` with the HUD composited |
