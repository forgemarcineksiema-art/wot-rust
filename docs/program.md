...
| X9 | **`parry3d` compiles for a function with zero callers**; `tank_footprints_intersect_query` is re-exported and never called | `physics/src/lib.rs:33`, `Cargo.toml:35` | the query and the dependency deleted (G6's half); the build graph one crate shorter |
...
| ID | Defect | Evidence | Closes when |
|---|---|---|---|
| J1 | **S is not the brake.** S = `Back` (throttle −1); the brake is Ctrl. A WoT veteran stops with S: engine P/v plus an accidental overspeed governor (the reverse cap 4.2 m/s), 2.9 s / 22.5 m from 50 km/h (T-54), then a tick past zero the hull REVERSES | `app/keybinds.rs:281-284`, `physics/src/forces.rs:96-106, 133-137` | opposing throttle while rolling routes through the brake channel; no reverse until the hull has stopped and S is pressed again; lock: S from 50 km/h stops in ≤ 1.9 s and never reverses |
| J2 | **The brake is a constant, not traction.** 4.8 m/s² everywhere, riverbed included (grip cap 1.8); the planner's `braking_deceleration_mps2()` claims `min(4.8, μ·g·grip)` and only bots read it | `forces.rs:94-95`, `sim/src/tank_drive.rs:163-168` | `brake_deceleration_mps2` 4.8 → 7.2 with `min(grip_long)`; derived: T-54 50→0 in ~1.75 s / ~12 m; `mobility_baseline.rs` brake column re-blessed; the bot helper truthful by construction |
| J3 | **Releasing W coasts 45 m.** `idle_drag` 1.3 + rolling 0.45 + kv² → 44.9 m / 6.9 s (T-54) | `forces.rs:108`, `controller_settings.rs:212` | `idle_drag_mps2` 1.3 → 2.6 (coast ~25 m); `movement_model.rs` lower bound 25 → ~15 m; replays with a W release re-pinned |
...
| J5 | **The heading overshoots 6.7° on every steer release** (yaw spool 0.30 s T-54, 0.58 s Tiger II); a pivot without throttle turns at half rate and creeps 0.51 m/s | `controller_settings.rs:198-199`, `movement.rs:153-156, 184-186` | spool clamp (0.25..0.7) → (0.12..0.35) or a 3× faster release decay; lock: overshoot ≤ 2°; the pivot table (`pivot_mechanism.rs`) untouched |
...
| S19 | **The reticle says Clear when the server buries the shell at the muzzle.** `trace_shell` returns `Expired` from `ground_contact` where the server emits `Terrain`; the client maps `Expired` → Clear | `sim/src/shell_trace/mod.rs:107-109`, `shell_step.rs:186-200`, `client/src/hud/reticle.rs:185` | `Obstacle{Terrain}` from the ground-contact branch; lock: a muzzle 0.2 m into a slope → server Terrain at tick 1 AND reticle Blocked |
...
| S22 | **A dead kernel with a real bug in the hot file.** `legacy_boxes.rs` lowers the deck by the radius instead of raising it; unreachable for the fleet (every kind has volumes); the ratchet still reads it | `sim/src/shell_trace/tank.rs:43-49`, `legacy_boxes.rs:19-28, 60-62`, `quality/tests/suite/combat_rules.rs:17` | deleted with the `armor_volumes: None` branch; lock: `vehicle_armor_volumes(kind).is_some()` for every `VehicleKind` |
