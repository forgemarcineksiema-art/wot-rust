...
| J8 | **The drawn barrel lags the shooting barrel.** The client's presentation spring (ω 7, ζ 0.55) smooths the replicated attitude and carries the fire/hit kicks, so the drawn hull nods ~1.1 s after the authoritative hull is at rest and up to ~3° behind the barrel the shell leaves | `engine/src/attitude.rs:12-14, 157` | the attitude tracks the replicated pose exactly (a pass-through), the kicks live in a separate decaying offset added on top; lock: at rest the presented pitch equals the replicated pitch to the bit, a kick decays without moving the rest |
