...
| 3.4 Bitwa | one life stands; the MODES and FORMATS are `docs/game-modes.md` (2026-09-06, row 22): PvP at 7v7 or 15v15 by the player's choice with marked bots in every empty seat, and an AI battle at 15v15; the objective (two bases, the moving target) and the timer stay product rows (R lane) |
...
| 3 | Maps 600 × 600 m on a 0.5 m heightfield (§3.4, §13.2) | 1000 × 1000 m at 2.5 m samples (5 m until T1, 2026-09-08), five shipped maps, the Terrain Atlas program | **The 1000 m format stays**; resolution rises where it is measured (T1: the 2.5 m ring). A new map may be authored at 600 m as an experiment against the "open book" finding |
...
| 15 (2026-09-05) | §15.1 "Liczby obrażeń pływające, kolor po typie" | S7: the damage number is demoted, the world promoted; A6: a zero is a word | **Both**: the numbers float at S7's size and life, coloured by OUTCOME family (pen / held / module / fire), never by shell type — the shell type is the hit log's job (H8, H9); and QUIET (the owner, the same evening: "nie natrętnie kolorowo, przesadnie") — one muted tone, no animation, no stacking. The same evening's rule for the reticle: "prosty, jak najmniej rozpraszaczy … jedynie istotne i ważne rzeczy" — §6's two elements and nothing that answers no question (H27) |
...
| 21 (2026-09-05) | §"Wygląd": „Materiały proceduralne … Kamuflaże z szumu per nacja. Oznaczenia taktyczne, numery, godła jednostek proceduralnie — czołg jest czyjś”; „Czołg na 400 m musi być do zobaczenia” | today the armour carries only the TEAM tint (`client/src/vehicle/pbr_mesh.rs`): every tank reads green or red, no nation paint, no scheme, no markings | **The owner (2026-09-05, night):** before any material or texture work a presentation bible per vehicle and nation, the same in garage, battle and studio (K24); the team colour becomes a separate read and never the paint. With it: every vehicle's tracks at the top bar (K22) and the guns reviewed as constructions (K23) |
| 22 (2026-09-06) | §2 "Format bitwy: 7v7, jedno życie, 7 minut" ("mecz potrzebuje 14 ludzi zamiast 30"); §3.4 "7v7"; §16 "kilkanaście meczów 7v7"; §17 "W PvP boty dopełniają do progu, jawnie oznaczone" | one format, a literal `7` in four files; the dedicated host seats humans on team one only (co-op against bots, never humans against humans); a lobby of "first seven, then bots" | **The owner (2026-09-06): two modes.** PvP in TWO formats by the player's choice — **7v7 or 15v15** — with bots in every seat the queue did not fill, marked (§17 stands); and an **AI battle at 15v15**, the player and bots, offline. One life stands. The population reasoning of §2 is kept as the fill deadline ("kolejka poniżej minuty"): the band never widens, the bots fill, the queue screen prints the bot share. The format becomes DATA (`game_core::BattleFormat` since M2), the queue is designed, 15v15 ships per map behind a 15-seat spawn gate and an MX330 measurement at thirty hulls — `docs/game-modes.md` (lane M). The same evening the owner accepted that document's ten queue rules R1–R10 and ruled documents only: no code before the word; and fixed three things — humans on BOTH teams in multiplayer with bots only in the empty seats (the mode's definition), the clocks 7 and 15 minutes, every budget sized for 14 tanks raised to the largest format with the optimisation that fits it |
| 23 (2026-09-06) | §3.4 and Załącznik A: the battle is 7 minutes | one time limit of 600 s (`RANDOM_BATTLE_TIME_LIMIT_S`, "ten minutes fits the 7v7 scale") — the design's clock was never implemented | **The clock is the format's row** (the owner, 2026-09-06 evening; `docs/game-modes.md` R8, finding 2): 7v7 = 7 minutes (420 s) as the document says, 15v15 = 15 minutes (900 s), the battle's time limit as DATA; M2 implements both clocks; the former 600 s on 7v7 is retired; a playtest may move either by a dated row |
| 24 (2026-09-06) | §2 "Przy 200 online w EU prime time", "Jeden serwer EU (Hetzner DE/FI)", §16 "Koszt hostingu przy grze płatnej pomijalny" | full 20 Hz snapshots, no delta: a saturated 30-tank snapshot is 8 029 B — 157 KiB/s per client; 1 000 clients would be ~1.3 Gbit/s, past one port and any fair-use line | **The owner (2026-09-06, evening): ~1 000 online, hosting CHEAP ("nie stać mnie"), and 15v15 "mega płynne" — no lag, no stutter, no FPS drops.** One Hetzner dedicated (AX41 class, ~40–50 €/month, 1 Gbit unmetered) carries it: the sim is ~2 % of a core per 15v15 battle and ~33 such battles are 1 000 online — but only with DELTA snapshots (netcode register row 9), which cut the wire 5–10× to ~150–250 Mbit/s at the peak. Delta snapshots are therefore a COST condition, not an optimisation; they move to the head of the netcode queue. The FPS half is M5b's cold measurement at thirty hulls per map, and its optimisation. And: Steam not for now (the same evening) — identity stays open, so rating (M8) waits on an identity decision |
| 25 (2026-09-07) | §4 „Rapier na kadłuby, kolizje czołg–czołg i propsy”, „Kadłub: jedna bryła sztywna, compound collider (kilka OBB lub convex hull)”, „Wysokość przeszkody, którą czołg przekracza, wynika ze skoku i punktu startu raycastu — nie z listy”; §3.3 „Jeśli fizyka pozwala wjechać na skałę, wjeżdżasz” | a planar SAT with no height and no cover rotation; cover blocks in plan at any height ("nothing ends up on a roof"); every town a 0°/90° grid; rocks are walls to the sky or ghosts | **The owner (2026-09-06): „Idziemy w 2,5D.”** Hull and cover are upright boxes with a yaw and a height band (XZ SAT ∧ y-interval, exact for upright boxes); a solid no taller than the step height taken from the running gear's geometry (`TrackShape.top_y`, the dossier's vertical obstacle as the acceptance number) is SUPPORT, a taller one blocks in its band; cover is an immovable body in the roster solver; §4's compound OBB is satisfied by the upright box; the step height is from geometry as §4 asks. Outside the model, deliberately: full 3D and Rapier (row 1 stands), hull-on-hull stacking, rollover from contact (the owner, 2026-09-07: „bez przewrotek czołgów” — the landing sway is not a flip), passing under solids, the gun as a body. `docs/program.md` lane X |
| 26 (2026-09-07) | §3.3 „Jeśli to psuje mapę, naprawiana jest mapa” | the map report walks the passable graph at the climb grade and knows nothing of the diagonal run-up (the envelope reaches 1.36 grade at 60° off square: the Prokhorovka and Ostrogorsk embankments are climbable) | **Climbing is skill; positions from it are never removed** (the owner, 2026-09-07, on WoT removing the Himmelsdorf and El Halluf climbs: „zabierali graczom rozrywkę i wiedzę/skilla”). „Naprawiana jest mapa” means the author KNOWS every climb (the report lists faces between 0.68 and 1.36 grade with their approach), never that a climb is walled off. P4.7 (terrain throws a track) rejected the same day. `docs/program.md` T10 |
| 27 (2026-09-07) | §3.3 „Czołg ma ważyć. Kołysanie kadłuba z zawieszenia mówi 45 ton”; §5/§6 the gun holds its aim | the sprung hull (G7) pitches 3.9° on a brake and the gun rides it 1:1 — `vertical_stabilizer` is 1.0 on the Centurion and 0.0 on every other hull; the gun marker and the impact X leave the ring for ~0.8 s on every stop and launch; S is throttle −1, the brake is Ctrl at a constant 4.8 m/s² (2.9 s / 22.5 m from 50 km/h on S, then a reverse); releasing W coasts 45 m; the heading overshoots 6.7° on a steer release | **The owner (2026-09-07): „trudno jest szybko wychamować, stanąć i wycelować w wroga. W World of Tanks jest inaczej” and „stabilizować udział nurka dla każdego działa jak w WoT”.** The dive share of the hull pitch is stabilised out of every gun (the Centurion keeps the terrain share, its historical stabiliser); the sway stays as the feel of 45 tonnes; S brakes and the brake is traction-limited (7.2 m/s² on grass, less in mud and water); releasing W engine-brakes (coast ~25 m); the steer spool is halved. Driving is the first block of the one queue. `docs/program.md` J1–J5 |
| 28 (2026-09-07) | §4 „Napęd: krzywa momentu × przełożenie, automatyczna skrzynia z punktami zmiany biegów. Daje wolno rusza, potem dociąga bez tuningu; moc/tona staje się statystyką, którą czuć” | the engine is one number (kW × 1.65 / mass, P/v with a 2.2 m/s floor: a 7.2 m/s² jolt for 0.35 s then a 1/v grind); the tracks program decided „No gearbox, no torque curve, no engine RPM” on 2026-08-06 | **The document's §4 stands: a gearbox as DATA per vehicle** (3–4 ratios, a flat torque curve, shift points; deterministic; the engine voice reads the shifts), in the driving block after the brakes, the mobility table re-recorded as a set. The 2026-08-06 decision is superseded (decision delegated to Claude, 2026-09-07). `docs/program.md` J6 |
| 29 (2026-09-07) | §13.4 „Zniszczenie per segment ściany, nie per budynek. Cztery stany”; §4 „Propsy: dwie klasy — bryły z masą i obiekty stanowe”; §12 „wieża ląduje jako prop”; §13.3 trees | one phase byte per whole box (intact / rubble / gone); a tree vanishes and a stump-and-log prism appears; a building swaps to a slab with 4–6 chunks under the tenement's dust; scenery never dies; no props with mass; the landed turret is a picture | **The owner (2026-09-07): „bez sensu … drzewo nie upada tylko jakby znika i pojawiają się jakieś boxy. Tak samo jest z budynkami”; and the taxonomy fences / boxes / signs / trees / walls / houses / terrain / mountains / big buildings.** Destruction becomes a DATA axis separate from the kind: Immovable / Stateful per wall segment (four states, the material deciding) / Breach / Topple (a fall-direction byte) / Crush / Prop (a knock-over state, no dynamics on the wire) / Terrain; trees and walls FALL as client choreography from the seed, ending in the authored state. Havok is the lesson: Wargaming showed full building physics in 2014 and shipped driving physics in 2016; buildings stayed authored states under dust. Buildings are finished BEFORE their collision (the owner). `docs/program.md` Z8–Z13, B1–B7 |
| 30 (2026-09-07) | §3.2 „Krzak, korona drzewa, dym z wraku, żywopłot — wolumeny z gęstością optyczną”; §15.3 „kto cię wykrył … przez krzak o gęstości 0,4”; the „Wygląd” chapter's camouflage; V2 in the second pass (camouflage per vehicle) | spotting is a binary LOS ray against cover boxes: a bush hides nothing, a tree line hides everything; no camouflage stat; view range 400/440 by era | **The owner (2026-09-07): „kamuflażów ja nie chcę, dokument projektowy kłamie. Oczywiście czołgi muszą mieć swój zasięg widzenia, wykrywanie itd, także zwiadowca ma swoją rolę” — and „krzaki mają chować”.** No camouflage stat, no camo paint bonus, no concealment factor per vehicle (V2 and the concealment half of R1 are out); §3.2 STANDS: foliage carries an optical density the sight line integrates (V1); an authored view range per vehicle from the dossier; the scout's role is view range, speed and position. Scenery still never blocks a shell. `docs/program.md` lane V |
| 31 (2026-09-07) | §4 „Pociski: swept raycast… Grawitacja tak, opór powietrza nie”; §6 „opór powietrza nie — spadek penetracji z dystansem tabelą” | the code has linear air drag per round (`c = 0.0130 / sectional density`: 0.07–0.12/s full-bore, 0.17–0.24/s tungsten, 0.05/s HEAT and HE) and penetration derived from it (`pen100 · (v(d)/v(100))^1.5`): a D-10T BR-412 at 1000 m arrives in 1.173 s with 6.64 m of drop and 162 of 185 mm; the APCR Pzgr 40/43 loses 21 % at 1 km (real tables ~19 %) | **The code stands** (decision delegated to Claude, 2026-09-07): one integrator for the server, the aim solver, the reticle preview and the renderer is worth more than a table, and the numbers match the real range cards within a few percent; the flight is locked numerically (drop and time of flight at 1000 m, the closed-form speed against the integrated one). §4 and §6 read „opór powietrza: liniowy, per pocisk; spadek penetracji z prędkości”. `docs/program.md` S14 |
| 32 (2026-09-07) | §6 „Rozrzut… mnożniki… zbiega wykładniczo”, the aim time as one time constant; §3.1 „obrażenia w funkcji grubości w punkcie” for HE; §3.1 „150 vs 149 wchodzi, zawsze” | the aim time is three e-folds (a „2.5 s” D-10T settles like WoT's 0.8 s); the bloom is additive and integrated (full speed 7.07 mrad, 2.44× base); HE without penetration is a flat 18 % of alpha whatever the plate (a T-34-85 side and a Tiger II glacis both take 77 HP); the dispersion seed is (tick, id, shot) and predictable; the casemate has zero traverse; the 60–70° glance band costs up to 30 % of penetration and the reticle shows the nominal figure | **Decided (delegated, 2026-09-07):** the three e-folds and the additive bloom stay and are on the record (the reticle side is not the bottleneck; the hull is, row 27); HE direct-hit damage becomes `max(0, 0.5·alpha − 1.3·LOS at the point)` — §3.1 implemented; the dispersion seed gets a per-battle salt from the server before the first public match; the casemate gets `Limited { half_arc, rate }` (the Jagdtiger ±10°); the effective penetration after the glance band is shown in the reticle hint; splash is occluded by wrecks; `r·u²` stays (row 5). `docs/program.md` S15–S18, S23 |
| 33 (2026-09-07) | §4 „Taranowanie: obrażenia z względnej energii kinetycznej ważonej stosunkiem mas. Ciężki przepycha średniego i to boli tylko średniego” | the bill is read from the solver's per-tick impulse while the speculative contact closes the remaining gap in the touch tick: the same charge bills ~268 or ~55 HP by 8 cm of spawn distance — a hidden die in a game whose rule 1 is no dice | **Billed from the peak closing speed per contact, once, and BOTH hulls pay, weighted by mass.** „Boli tylko średniego” is amended: a free weapon for heavies is a bot-spam vector at 15v15; the heavy pays a little, as the mass ratio says. Wrecks can be pushed (parking friction, never driven). `docs/program.md` X7, X12 |
| 34 (2026-09-07) | §13.4 „Okna i drzwi to otwory w proxy kolizji i wzroku — linia wzroku przez okno działa bez przypadku specjalnego” | boxes have no holes; sight and shell treat a wall as a slab | **Windows are not apertures for SIGHT** (delegated, 2026-09-07): at 20 Hz snapshots and a 5 m grid a window shot is „trafił mnie przez ścianę” for the victim and a spot through two windows in line for the observer. A window is thin wood in the same thickness table (a shell pays it and passes with penetration loss); the sight line is blocked. Revisited only after a playtest. `docs/program.md` decision 14 |
| 35 (2026-09-07) | §13.2 „prymitywy taktyczne jako pędzle — grzbiet, niecka, korytarz, nasyp”; §13.5 the map elements; §3.4 the battle | the blueprint knows five point roles (Observation, HullDown, Crossing, FlankRoute, HighGround); lanes, crossfire, sniper perches, rotation paths and fallback positions are prose in the dossiers; exposure lives in a tool, not in the report; bots steer straight and never path around cover | **The owner (2026-09-07): maps are first of all „lanes, crossfire, cover, hull-down, flanking, sniper positions, rotation paths, fallback positions (cała gameplay topology)”.** The roles become data (append-only) and the map report computes and goldens them from geometry; bots route on that graph; the representative fragment (Kamienna on Bystra, the T-54, ~10 s: parapet → bluff → market lane → shot → breach) is the acceptance frame. The terrain grid goes to 2.5 m map-wide with chunk LOD and geomorphing, gated by the map-swap and frame measurements (row 3's „resolution rises where it is measured”). `docs/program.md` lane W, T1 |
| 36 (2026-09-09) | §2 minimum hardware/60 FPS, row 2's MX330 floor and the one-look rule | A canonical picture exists, but the owner reports worsening FPS through the battle, preventing aiming; recorded 2026-09-08 runs exceed the budget | **The owner: stable 60 FPS on the MX330 laptop is sufficient; stronger PCs should deliver more FPS with one intended look on every supported PC.** Preserve combat visibility, world presence and readability; no quality preset may remove cover/foliage for an advantage. Resolution and FPS may differ after validation. Sustained late-battle control is the priority, not startup/average FPS. The owner clarified the same day that all activities are affected; aiming was an example, not a diagnosed trigger. This is an accepted target, not achieved performance. [One-look policy](one-look-policy.md), [sustained-performance plan](sustained-performance-plan.md); `program.md` Q11–Q16. Numerical acceptance thresholds are the plan's engineering targets, not measurements or a verbatim owner instruction |

The document's largest content gap is not in this table: "content robi pipeline" (30–40 tanks
