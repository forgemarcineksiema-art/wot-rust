//! The hangar's upper band: the set pieces that make the nave read as a working maintenance
//! facility instead of an empty box — a catwalk gallery with its stair, crane rails the
...
use super::hangar::{Finish, HALF_X, HALF_Z, SLAB, WALL_HEIGHT, finish, slab};
...
/// Height of the crane girder's centreline: just under the truss chords at [`WALL_HEIGHT`],
/// with its cross rail on top still clearing them. Shared by the girder (`hangar_props`) and
/// the wall rails it rides, so the crane cannot ride above its own track.
pub(super) const CRANE_GIRDER_Y: f32 = 8.3;
/// Where the worklamps hang — must agree with `SceneLighting::garage_hero()`'s rig (locked by
/// `lamp_faces_run_hot_and_hang_from_housings`). Dropped with the roof: the old hall hung them
/// at 9.8 m under a 12.6 m lid; a 9 m chord puts the high bays at 7.6.
pub(super) const HIGH_BAY_LAMPS: [[f32; 3]; 2] = [[-3.6, 7.6, 1.8], [3.6, 7.6, -1.8]];
pub(super) const STORES_LAMP: [f32; 3] = [-8.5, 6.2, 17.0];
pub(super) const SECOND_BAY_LAMP: [f32; 3] = [-5.5, 7.4, 14.5];
pub(super) const BENCH_STRIP_LAMP: [f32; 3] = [9.2, 3.4, 6.0];
...
/// clear) turning down the left wall, kick plates, railing, support columns, and a stair
/// flight descending toward the stores zone. The decks ABUT at the corner (never overlap —
/// coplanar top faces would z-fight).
fn push_catwalk(v: &mut Vec<SceneVertex>, i: &mut Vec<u32>) {
    let start = v.len();
    let deck_c = DECK_Y - 0.06;
    let inner_z = HALF_Z - 0.25 - 2.2; // inner edge plane of the 2.2 m-deep end-wall deck
    let inner_x = HALF_X - 0.25 - 2.2; // inner edge plane of the left-wall leg
    // End-wall run spans the full width; the left-wall leg starts where the end run ends and
    // reaches down toward the stores zone's stair.
    slab(v, i, [0.0, deck_c, HALF_Z - 1.35], [HALF_X - 0.25, 0.06, 1.1], CATWALK_DECK);
    let leg_z = (inner_z + 13.0) / 2.0;
    let leg_half = (inner_z - 13.0) / 2.0;
    slab(v, i, [-(HALF_X - 1.35), deck_c, leg_z], [1.1, 0.06, leg_half], CATWALK_DECK);

    // Kick plates + two-rail railing along both inner edges.
    let rail_runs: [([f32; 3], [f32; 3]); 2] = [
        ([0.0, 0.0, inner_z], [HALF_X - 0.25, 0.0, 0.03]),
        ([-inner_x, 0.0, leg_z], [0.03, 0.0, leg_half]),
    ];
    ...
    let posts = ((HALF_X - 0.25) * 2.0 / 2.2) as i32;
    for k in 0..=posts {
        let x = -(HALF_X - 0.25) + k as f32 * 2.2;
        if x <= HALF_X - 0.25 {
            post(x, inner_z);
        }
    }
    ...
    for k in 0..=leg_posts {
        let z = 13.0 + k as f32 * 2.2;
        if z <= inner_z {
            post(-inner_x, z);
        }
    }
    let col_half = (DECK_Y - 0.12) / 2.0;
    for k in 0..=((HALF_X - 1.0) / 4.4) as i32 {
        for x in [-(k as f32) * 4.4, k as f32 * 4.4] {
            slab(v, i, [x, col_half, inner_z + 1.0], [0.09, col_half, 0.09], COLUMN);
        }
    }

    // The stair: ten steps descending from the leg's near end toward the stores zone.
    for step in 0..10 {
        let k = step as f32;
        slab(
            v,
            i,
            [-(HALF_X - 1.35), DECK_Y - 0.46 * (k + 1.0) + 0.045, 13.0 - 0.30 * (k + 1.0)],
            [0.55, 0.045, 0.14],
            CATWALK_DECK,
        );
    }
...
/// overhead girder finally rides something, the whole length of the nave.
fn push_crane_rails(v: &mut Vec<SceneVertex>, i: &mut Vec<u32>) {
    let start = v.len();
    let rail_y = CRANE_GIRDER_Y + 0.34 + 0.16; // just under the girder's cross rail
    for sign in [-1.0_f32, 1.0] {
        let x = sign * (HALF_X - 1.1);
        slab(v, i, [x, rail_y, 0.0], [0.14, 0.16, HALF_Z - 0.6], RAIL);
        for k in [-0.8_f32, -0.4, 0.0, 0.4, 0.8] {
            let bracket_x = sign * (HALF_X - 0.62);
            slab(v, i, [bracket_x, rail_y - 0.05, k * HALF_Z], [0.48, 0.10, 0.28], COLUMN);
        }
    }
...
fn push_worklamps(v: &mut Vec<SceneVertex>, i: &mut Vec<u32>) {
    ...
    slab(
        v,
        i,
        [(x + HALF_X - SLAB) / 2.0, y + 0.3, z],
        [(HALF_X - SLAB - x) / 2.0, 0.04, 0.04],
        LAMP_STEM,
    );
...
fn push_signage(v: &mut Vec<SceneVertex>, i: &mut Vec<u32>) {
    ...
    slab(v, i, [0.0, WALL_HEIGHT - 2.0, banner_z], [1.6, 0.9, 0.03], BANNER);
    for (dy, half) in [(0.82_f32, [1.52, 0.06, 0.035]), (-0.82, [1.52, 0.06, 0.035])] {
        slab(v, i, [0.0, WALL_HEIGHT - 2.0 + dy, banner_z], half, BANNER_BORDER);
    }
    for dx in [-1.52_f32, 1.52] {
        slab(v, i, [dx, WALL_HEIGHT - 2.0, banner_z], [0.06, 0.9, 0.035], BANNER_BORDER);
    }
