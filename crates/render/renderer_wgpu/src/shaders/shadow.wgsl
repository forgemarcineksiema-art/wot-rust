...
struct VsDepthOutCutout {
    ...
    @location(3) @interpolate(flat) dither: vec2<f32>,
};

fn depth_out_cutout(clip: vec4<f32>, world_pos: vec3<f32>, damage_index: u32, uv: vec2<f32>, dither: vec2<f32>) -> VsDepthOutCutout {
...
fn vs_main_cutout(input: VsInCutout) -> VsDepthOutCutout {
    ...
    return depth_out_cutout(camera.light_view_proj * world, world.xyz, input.damage_index, input.uv, input.dither);
}

@vertex
fn vs_far_cutout(input: VsInCutout) -> VsDepthOutCutout {
    let world = cutout_caster_world(input);
    return depth_out_cutout(camera.light_view_proj_far * world, world.xyz, input.damage_index, input.uv, input.dither);
}

@vertex
fn vs_prepass_cutout(input: VsInCutout) -> VsDepthOutCutout {
    let world = cutout_caster_world(input);
    return depth_out_cutout(camera.view_proj * world, world.xyz, input.damage_index, input.uv, input.dither);
}
...
fn fs_depth_cutout(input: VsDepthOutCutout) {
    ...
    // A light-space texel is not a screen pixel: of one tree's objects, the one whose window
    // holds the threshold 0.5 casts the shadow alone — exactly one per tree, always.
    if (!(input.dither.x <= 0.5 && 0.5 < input.dither.y)) {
        discard;
    }
}
