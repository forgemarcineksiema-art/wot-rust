...
fn tank_snapshot_wire_v34_is_stable() {
    ...
    assert_eq!(PROTOCOL_VERSION, 54);
    assert_eq!(hex(&bytes), wire_fixture(&bytes, "snapshot_tank_v54"));
...
fn combat_snapshot_wire_v34_is_stable() {
    ...
    assert_eq!(PROTOCOL_VERSION, 54);
    assert_eq!(hex(&bytes), wire_fixture(&bytes, "snapshot_combat_v54"));
    assert_eq!(decode_message(&bytes).expect("snapshot should decode"), message);
}
...
fn snapshot_delivery_wire_v38_is_stable() {
    ...
    assert_eq!(hex(&bytes), wire_fixture(&bytes, "snapshot_delivery_v54"));
...
/// v52 (interface program W-7): the battle's end carries the recipient's spotting log.
#[test]
fn the_battle_ended_word_with_its_spotting_log_wire_v52_is_stable() {
    let ended = ProtocolMessage::BattleEnded {
        session_id: SESSION_ID,
        winning_team: Some(2),
        spotting_log: vec![
            net::SpottingRecord {
                observer: game_core::TankId(9),
                distance_m: 308.5,
                from_tick: 40,
                to_tick: 96,
            },
            net::SpottingRecord {
                observer: game_core::TankId(11),
                distance_m: 142.0,
                from_tick: 120,
                to_tick: 121,
            },
