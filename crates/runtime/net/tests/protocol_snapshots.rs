...
fn tank_snapshot_wire_v34_is_stable() {
    ...
    assert_eq!(PROTOCOL_VERSION, 55);
    assert_eq!(hex(&bytes), wire_fixture(&bytes, "snapshot_tank_v55"));
...
fn combat_snapshot_wire_v34_is_stable() {
    ...
    assert_eq!(PROTOCOL_VERSION, 55);
    assert_eq!(hex(&bytes), wire_fixture(&bytes, "snapshot_combat_v55"));
    assert_eq!(decode_message(&bytes).expect("snapshot should decode"), message);
}
...
fn snapshot_delivery_wire_v38_is_stable() {
    ...
    assert_eq!(hex(&bytes), wire_fixture(&bytes, "snapshot_delivery_v55"));
