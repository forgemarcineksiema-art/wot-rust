...
impl SceneRenderer {
    ...
    /// Replace the baked static scene geometry (the "terrain" slot). Recreates the vertex and
    /// index buffers — cheap because it only happens on a scene change (e.g. garage <-> battle),
    /// not per frame. Lets one renderer host both the battlefield and the garage hangar.
