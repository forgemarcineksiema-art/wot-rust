...
/// units, tinted `color`. Each visible glyph becomes one textured `HudVertex` quad.
...
mod tests {
    ...
    #[test]
    fn text_width_grows_with_more_glyphs_and_emits_six_verts_per_glyph() {
        let aspect = 16.0 / 9.0;
        ...
        assert_eq!(verts.len(), 12, "two visible glyphs => two textured quads");
    ...
    fn right_aligned_text_ends_at_the_anchor() {
        let aspect = 16.0 / 9.0;
        let mut verts = Vec::new();
        push_text_right(&mut verts, "42", 0.8, 0.0, 0.05, aspect, [1.0, 1.0, 1.0, 1.0]);
