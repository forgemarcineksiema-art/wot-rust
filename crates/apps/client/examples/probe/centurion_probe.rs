...
/// `cargo run -p client --example probe -- centurion_probe -- out.png`
pub(crate) fn run() -> Result<(), Box<dyn std::error::Error>> {
    let path = crate::sub_arg(1).unwrap_or_else(|| "target/centurion_probe.png".to_string());
    let width = 1280u32;
    let height = 720u32;
    ...
    let snapshot = TankSnapshot {
        ...
        module_hit_points: VehicleKind::Centurion.spec().module_health.hit_points_by_slot(),
