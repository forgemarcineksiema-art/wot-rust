...
/// `cargo run -p client --example probe -- t54_bow_probe -- out.png`
pub(crate) fn run() -> Result<(), Box<dyn std::error::Error>> {
    let path = crate::sub_arg(1).unwrap_or_else(|| "target/t54_bow_probe.png".to_string());
    let width = 1280u32;
    let height = 720u32;
    ...
    let camera = Camera {
        eye: [cx + 3.6, ground + 1.9, cz + 5.2],
        target: [cx - 0.4, ground + 1.5, cz + 0.6],
