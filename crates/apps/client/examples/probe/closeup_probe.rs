...
/// The model-logic gate's close-up sweep: low front, close flank, and close turret shots for
/// every fleet vehicle — the angles where floaters, interpenetrations, closed openings, and
/// scale absurdities show. Complements deck_probe (raised rear-¾/front-¾ deck reads).
/// `cargo run -p client --example probe -- closeup_probe -- <out_dir>`
pub(crate) fn run() -> Result<(), Box<dyn std::error::Error>> {
    let out_dir = crate::sub_arg(1).unwrap_or_else(|| "target/closeup_probe".to_string());
    ...
    for (kind, slug) in fleet {
        ...
        // Low front (bow logic), close flank (hull side + gear), close raised turret (roof
        // furniture at reading distance). Turret yawed 0.6 rad so the mantlet/ring seat shows.
        let views = [
            ("front_low", [cx - 3.0, ground + 1.1, cz + 8.5], [cx, ground + 1.6, cz]),
            ("flank", [cx + 7.5, ground + 1.6, cz + 2.0], [cx, ground + 1.4, cz]),
            ("turret", [cx + 4.2, ground + 3.6, cz + 4.2], [cx, ground + 2.2, cz]),
        ];
