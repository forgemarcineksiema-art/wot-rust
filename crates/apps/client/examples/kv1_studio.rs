...
/// A tight, single-vehicle right-profile studio render of the KV-1 through the PBR catalog path —
/// the same path the garage uses, with the baked Forge artifacts in `target/forge` loaded so the
/// shot shows the real textured material (falling back to the live bake + neutral material if no
/// artifact is present). This is the KV's step (h) verification render: the profile that judges
/// the slab-sided loaf against the fleet's domes, the fender shelves bridging out over the six
/// even wheel stations, the three return rollers carrying a taut top run, and the spoked idler
/// against the toothed sprocket at the tail.
/// `cargo run -p client --example kv1_studio -- out.png`
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let path = std::env::args().nth(1).unwrap_or_else(|| "target/kv1_studio.png".to_string());
    let width = 1280u32;
    let height = 720u32;
    ...
    let snapshot = TankSnapshot {
        ...
        module_hit_points: VehicleKind::KV1_1942.spec().module_health.hit_points_by_slot(),
    ...
    let camera = Camera {
        eye: [cx, ground + 2.1, cz + 9.5],
        target: [cx, ground + 1.05, cz],
