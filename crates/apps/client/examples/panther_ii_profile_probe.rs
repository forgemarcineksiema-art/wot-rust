...
/// `cargo run -p client --example panther_ii_profile_probe -- out.png`
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let path = std::env::args().nth(1).unwrap_or_else(|| "target/panther_ii_profile_probe.png".to_string());
    let width = 1280u32;
    let height = 720u32;
    ...
    let snapshot = TankSnapshot {
        ...
        module_hit_points: VehicleKind::PantherII.spec().module_health.hit_points_by_slot(),
