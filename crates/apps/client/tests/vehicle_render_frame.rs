...
/// Cached meshes: hull/turret/gun for every vehicle, plus eleven unit gear meshes per blueprint
/// one (road wheel, swing arm, damper and the idler's tension crank with their mirrored LEFT
/// hands, sprocket, idler, track link, return roller) at EACH of the two detail tiers — near and
/// distant are registered together at load.
fn expected_mesh_count() -> usize {
    VehicleKind::ALL
        .iter()
        .map(|kind| {
            3 + if RunningGearKinematics::for_vehicle(*kind).is_some() { 11 * 2 } else { 0 }
        })
        .sum()
}
...
fn pbr_render_stops_only_the_broken_track_side() {
    ...
    // Wheels and arms, dampers, sprocket + idler + the idler's crank, links.
    let right_count = kin.wheel_zs.len() * 2 + kin.damper_stations.len() + 3 + kin.link_count();
