...
fn bot_command_for_tank(
    ...
    if let Some(target) = bot_nearest_visible_enemy(tank, tanks, battlefield) {
    ...
    tanks
        ...
        .filter(|target| {
            target.team != tank.team
                ...
                && bot_has_los(tank, target, battlefield)
        })
        .min_by(|a, b| {
            tank.position
...
fn bot_has_los(tank: &TankState, target: &TankState, battlefield: &BattlefieldMap) -> bool {
    let from = bot_observer_eye(tank);
    bot_target_points(target).iter().any(|&point| {
        sim::line_of_sight(Some(&battlefield.heightmap), &battlefield.static_cover, from, point)
    })
}

fn bot_observer_eye(tank: &TankState) -> Vec3 {
    tank.position + Vec3::Y * (tank.spec.hitbox.center_y_m + tank.spec.hitbox.half_height_m)
}

fn bot_target_points(tank: &TankState) -> [Vec3; 2] {
    [
        tank.position + Vec3::Y * tank.spec.hitbox.center_y_m,
        tank.position + Vec3::Y * (tank.spec.hitbox.center_y_m + tank.spec.hitbox.half_height_m),
    ]
}
